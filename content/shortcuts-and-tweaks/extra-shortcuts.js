		...
		if (targetNode) {
			if (FoxtrickPrefs.isModuleOptionEnabled("ExtraShortcuts", "Supporterstats")
				...
				if (FoxtrickPrefs.isModuleOptionEnabled("ExtraShortcuts", "FoxTrickPrefs")) {
					...
					link.addEventListener('click', function() {FoxtrickPrefs.show('#tab=on_page&view-by=page');}, false);
					var img1 = doc.createElement('img');
			...
			for(i=0; i<this.RADIOS.length; ++i) {
				...
				if (FoxtrickPrefs.isModuleOptionEnabled("ExtraShortcuts", radio)) {
					...
					if (FoxtrickPrefs.isModuleOptionEnabled("ExtraShortcuts", "AddLeft")) targetNode.insertBefore(span,targetNode.firstChild);
					else {
						if (targetNode.lastChild.nodeName=='BR') {
