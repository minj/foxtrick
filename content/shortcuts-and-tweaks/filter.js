...
Foxtrick.modules["Filter"]={
	...
	run : function(doc) {
		...
		var FILTER_VAL = {
			"statsTopPlayers" : 
			{
				toBeFiltered: {nodeName:"table", index:2, rowType:'tr', cellType:'td', rowStartIndex:1}, 
					...
					getFilters(page, function(filters) {
						for (var j = 0; j < filters.length; ++j) {
							...
							if (filter.filtertype == "minmax") {
								...
								doc.getElementById("Filters." + filter.key + ".check").removeAttribute("checked");
							}
							else if (filter.filtertype == "category") {
								filters[j].category = null;
								doc.getElementById("Filters." + filter.key + ".category").selectedIndex = 0;
							}
						}
					});
				});
					...
					for (var i = FILTER_VAL[page].toBeFiltered.rowStartIndex; i < list.length; ++i) {
						...
						for (var j = 0; j < filters.length; ++j) {
							...
							if (val===null || !filter.filtertype)
								continue
							else if (filter.filtertype == "minmax" && (filter.min != null || filter.max != null) ) {
								...
								if (filter.category && FILTER_FUNC[filter.filtertype](val, filter.category))
									hide = true;
							}
