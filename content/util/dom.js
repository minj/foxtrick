...
// there would be a workaround for domattrchanged if realy needed
// (keep a copy of the node attribs and check for changes every second)
...
Foxtrick.addMutationEventListener = function(target, type, listener, useCapture) {
	...
	// attach an alternative handler which could get executed several times for
	// multiple changes at once. Since our handler should execute only once,
	// we use a different handler to call ours when all changes have past
	var DOMListener = new Foxtrick.DOMListener(target, type, listener, useCapture);
	Foxtrick.MutationEventListeners.push({ target: target, type: type, listener: listener,
		useCapture: useCapture, DOMListener: DOMListener });
	DOMListener.start();
};
