...
// </summary>
// only allow harmless tags, no attributes other than maybe title, alt, width & height and save href/src for a & img
Foxtrick.util._tags = new RegExp("<[^>]*(>|$)",'g');
Foxtrick.util._whitelist = new RegExp("^</?(b(lockquote)?|code|d(d|t|l|el)|em|h(1|2|3)|i|kbd|li|ol|p(re)?|s(ub|up|trong|trike)?|ul)>$|^<(b|h)r\s?/?>$");
Foxtrick.util._whitelist_a = new RegExp("^<a\\shref=\"(#[-a-z0-9_]+|(https?|ftp):\/\/[-a-z0-9+&@#\/%\?=~_\|!:,.;\(\)]+)\"(\\stitle=\"[^\"<>]+\")?\\s?>$|^<\/a>$");
Foxtrick.util._whitelist_img = new RegExp("^<img\ssrc=\"https?:\/\/[-a-z0-9+&@#\/%\?=~_\|!:,.;\(\)]+\"(\\swidth=\"\\d{1,3}\")?(\\sheight=\"\\d{1,3}\")?(\\salt=\"[^\"<>]*\")?(\\stitle=\"[^\"<>]*\")?\\s?/?>$");
