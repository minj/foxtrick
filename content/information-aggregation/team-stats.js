...
//Foxtrick.log('playerList',playerList)
			...
			if (FoxtrickPrefs.isModuleOptionEnabled("TeamStats", "General")) {
				...
				if (totalAge) {
					...
					addRow("TeamStats.General", Foxtrickl10n.getString("Age"), avgYears + "." + avgDays);
				}
				if (Foxtrick.Pages.Players.isYouthPlayersPage(doc)) {
					var youngerThanNineteen = numPlayers - olderThanNineteen;
					var row = addRow("TeamStats.General", Foxtrickl10n.getString("TeamStats.PlayerNotToOld"), youngerThanNineteen);
					if (youngerThanNineteen < 9) {
						row.className = "red";
					}
					if (olderThanNineteen) {
						var row = addRow("TeamStats.General", Foxtrickl10n.getString("TeamStats.PlayerToOld"), olderThanNineteen);
						row.className = "red";
					}
				}
			}
			...
			if (FoxtrickPrefs.isModuleOptionEnabled("TeamStats", "Speciality")) {
				if (hasSpecialities) {
					...
					for (var i in specSummary) {
						addRow("Speciality", specSummary[i].type, specSummary[i].count, "speciality-" + Foxtrickl10n.getEnglishSpeciality(specSummary[i].type), specSummary[i].type);
					}
				}
			}
			if (FoxtrickPrefs.isModuleOptionEnabled("TeamStats", "Personality")) {
				if (Foxtrick.Pages.Players.isPropertyInList(playerList, "aggressiveness")
					...
					addRow("Personality", Foxtrickl10n.getString("Aggressiveness"), Foxtrickl10n.getLevelByTypeAndValue("aggressiveness", avgAggressiveness));
					addRow("Personality", Foxtrickl10n.getString("Agreeability"), Foxtrickl10n.getLevelByTypeAndValue("agreeability", avgAgreeability));
					addRow("Personality", Foxtrickl10n.getString("Honesty"), Foxtrickl10n.getLevelByTypeAndValue("honesty", avgHonesty));
				}
			}
			if (FoxtrickPrefs.isModuleOptionEnabled("TeamStats", "Status")) {
				if (transferListed > 0) {
					...
					addRow( "Status", img, transferListed, "transfer-listed", Foxtrickl10n.getString("TeamStats.TransferListed"));
				}
				if (yellowCards > 0) {
					var img = doc.createElement("img");
					...
					addRow( "Status", img, yellowCards, "cards",Foxtrickl10n.getString("TeamStats.Cards"));
				}
				if (twoYellowCards > 0) {
					var img = doc.createElement("img");
					...
					addRow( "Status", img, twoYellowCards, "cards",Foxtrickl10n.getString("TeamStats.Cards"));
				}
				if (redCards > 0) {
					var img = doc.createElement("img");
					...
					img.className = "cardsOne";
					addRow( "Status", img, redCards, "cards",Foxtrickl10n.getString("TeamStats.Cards"));
				}
				if (bruised > 0) {
					var img = doc.createElement("img");
					...
					addRow( "Status", img, bruised, "injured",Foxtrickl10n.getString("TeamStats.Injured"));
				}
				if (injured > 0) {
					var img = doc.createElement("img");
					...
					addRow( "Status", img, data, "injured",Foxtrickl10n.getString("TeamStats.Injured"));
				}
			}
				...
				|| Foxtrick.Pages.Players.isPropertyInList(playerList, "countryId")) {
					...
					for (var i in leagues) {
						if (isLeague)
							...
							var name = Foxtrick.XMLData.League[Foxtrick.XMLData.countryToLeague[i]].LeagueName;
