		...
		var handleHyResponse = function(response, status) {
			switch (status) {
				...
				case 200:
					...
					break;
				case 400:
					Foxtrick.log('YouthSkills: Given data is invalid or incomplete',
					             status, response);
					doSkillColoring(doc);
					return;
				case 404:
			...
			var setMaxSkill = function(playerId, skill, value) {
				...
				if (value) {
					...
					Foxtrick.addClass(maxLink, 'ft-youthskills-link');
			...
			var setCurrentSkill = function(playerId, skill, value, max, maxed) {
				...
				if (value) {
					...
					Foxtrick.addClass(minLink, 'ft-youthskills-link');
					//used to signal skillcoloring that we manually trigger coloring
