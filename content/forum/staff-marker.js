...
Foxtrick.modules['StaffMarker'] = {
	...
	run: function(doc) {
		...
		if (Foxtrick.isPage(doc, 'forumViewThread') &&
			...
			FoxtrickPrefs.isModuleOptionEnabled('StaffMarker', 'forumInterface'))
	...
	addForumInterface: function(doc) {
		var module = this;
		var markUser = Foxtrickl10n.getString('StaffMarker.userColor');
		var fgColor = Foxtrickl10n.getString('StaffMarker.textColor');
		var bgColor = Foxtrickl10n.getString('StaffMarker.bgColor');
		var saveText = Foxtrickl10n.getString('button.save');
		var closeText = Foxtrickl10n.getString('button.close');
		var resetText = Foxtrickl10n.getString('button.reset');
		...
		for (var i = 0; i < elems.length; i++) {
			...
			Foxtrick.onClick(btnSave, (function(i, userId) {
				return function(ev) {
					...
					FoxtrickPrefs.setString('module.StaffMarker.own_text',
											 JSON.stringify(customMarker));
					FoxtrickPrefs.setModuleEnableState('StaffMarker.own', true);
					ev.preventDefault();
				};
			})(i, userId));
