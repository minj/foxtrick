...
Foxtrick.modules["EmbedMedia"]={
	...
	run : function(doc) {
		...
		var doEmbedActualImageUrl = function(target){
			...
			Foxtrick.addImage(doc, target.nextSibling.firstChild, {src:target.nextSibling.firstChild.href, title: title, alt: title, style:'max-width:99.9%'});
		}
				...
				if(link.href.indexOf('format=json') == -1){ //check very rare case, like when explaining what oembed does and posting example url
					for (var key in filter_supported)
					{
						...
						if( matches ){
							...
							//dunno about safari
							if(key == "imageshack" && (Foxtrick.platform == "Opera" || Foxtrick.platform == "Safari"))
								continue;
