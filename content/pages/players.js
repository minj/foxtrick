...
Foxtrick.Pages.Players = {
	...
	getTransferPlayerList : function(doc) {
		try {
			Foxtrick.dump('getTransferPlayerList\n');
			...
			for (var i=0; i<TransferPlayers.rows.length; ) {
				...
				player.deadline = TransferPlayers.rows[i+6].cells[1].getElementsByTagName('span')[0].cloneNode(true);;
				
				var overviewtable = TransferPlayers.rows[i].getElementsByTagName("table")[0];
				...
				if (bidderlink.length!==0) {
					...
					player.currentBidderLinkShort.innerHTML='x';
				}
				player.currentBid = Foxtrick.trimnum(overviewtable.rows[0].cells[4].innerHTML);
				
				var nameLink = overviewtable.rows[0].cells[1].getElementsByTagName('a')[0];
				player.id = nameLink.href.replace(/.+playerID=/i, "").match(/^\d+/)[0];
				player.nameLink = nameLink.cloneNode(true);
				player.countryId = Foxtrick.XMLData.getCountryIdByLeagueId(TransferPlayers.rows[i].getElementsByTagName("a")[0].href.match(/leagueId=(\d+)/i)[1]);
				
				player.ageText = TransferPlayers.rows[i+3].cells[1].innerHTML;
				var ageMatch = player.ageText.match(/(\d+)/g);
				player.age = { years: parseInt(ageMatch[0]), days: parseInt(ageMatch[1]) };				
				player.tsi = parseInt(Foxtrick.trimnum(TransferPlayers.rows[i+4].cells[1].innerHTML));			
				player.speciality = Foxtrick.trim(TransferPlayers.rows[i+5].cells[1].innerHTML);
				if (player.speciality=='-') player.speciality='';
				
				var links = {};
				var basicSkillLinks = TransferPlayers.rows[i+1].cells[0].getElementsByTagName('a');
				links.form = basicSkillLinks[2];
				links.stamina = TransferPlayers.rows[i+2].cells[3].getElementsByTagName('a')[0];
				...
				for (var j = 0; j < skillOrder.length; ++j) {
					var level = TransferPlayers.rows[i+skillOrderRow[j]].cells[skillOrderCell[j]].getElementsByTagName("a")[0].href.match(/ll=(\d+)/)[1];
					player[skillOrder[j]] = parseInt(level);
				}
