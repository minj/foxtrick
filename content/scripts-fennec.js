...
Foxtrick.loader.gecko.contentScripts = [
		//<!-- essential stuffs -->
...
Foxtrick.loader.gecko.fennecContentScriptLoad = function(event) {
	try { 
		// run only once
		//removeEventListener("UIReady", Foxtrick.loader.gecko.fennecContentScriptLoad, false);

		for (var i=0; i<Foxtrick.loader.gecko.contentScripts.length; ++i) 
			messageManager.loadFrameScript(Foxtrick.loader.gecko.contentScripts[i], true);

		Foxtrick.log('content scripts load');
	} catch (e) {
		Foxtrick.log('fennecScript content script load', e);
	}
};

Foxtrick.loader.gecko.fennecContentScriptUnload = function() {
	try { 
		for (var i=0; i<Foxtrick.loader.gecko.contentScripts.length; ++i) 
			messageManager.removeDelayedFrameScript(Foxtrick.loader.gecko.contentScripts[i]);

		Foxtrick.log('content scripts unloaded');
	} catch (e) {
		Foxtrick.log('fennecScript content script unload', e);
	}
};
