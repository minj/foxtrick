...
(function() {
	...
	var eventTypes = {
		...
		'58': 'miss',
		'59': 'miss',
		'60': { 'team': ['underestimation'] },
		'61': 'confusion',
		'62': 'pullback',
		'63': { 'team': ['remove_underestimation'] },
		'65': 'nerves',
		'66': { 'team': ['remove_underestimation_0'] },
		'67': { 'team': ['remove_underestimation_1'] },
		'68': 'pressing',
		'70': 'extraTime',
		'71': 'penaltyShootOut',
		'73': 'tossing_coin',
	...
	var icons = {
		...
		'underestimation': Foxtrick.InternalPath + 'resources/img/matches/thumbs_down.png',
		'remove_underestimation': Foxtrick.InternalPath + 'resources/img/matches/thumbs_up.png',
		'remove_underestimation_0': Foxtrick.InternalPath + 'resources/img/matches/thumbs_down-down.png',
		'remove_underestimation_1': Foxtrick.InternalPath + 'resources/img/matches/thumbs_up-down.png',
		...
		run: function(doc) {
			...
			if (FoxtrickPrefs.getBool('anstoss2icons')) {
				...
				icons['se_technical'] = Foxtrick.InternalPath +
					'resources/img/matches/spec1_alt.png';
				...
				icons['se_unpredictable'] = Foxtrick.InternalPath +
					'resources/img/matches/spec4_alt.png';
				icons['se_head_specialist_negative'] = Foxtrick.InternalPath +
					'resources/img/matches/spec5_red_alt.png';
				icons['se_technical_negative'] = Foxtrick.InternalPath +
					'resources/img/matches/spec1_red_alt.png';
				icons['se_powerful_negative'] = Foxtrick.InternalPath +
					'resources/img/matches/spec3_red_alt.png';
				icons['se_quick_negative'] = Foxtrick.InternalPath +
					'resources/img/matches/spec2_red_alt.png';
				icons['se_unpredictable_negative'] = Foxtrick.InternalPath +
					'resources/img/matches/spec4_red_alt.png';
			}
			...
			var playerTag = function(id, name) {
				...
				link.href = isYouth ? '/Club/Players/YouthPlayer.aspx?YouthPlayerID=' + id :
					'/Club/Players/Player.aspx?playerId=' + id;
				  ...
				  function(homeXml) {
					Foxtrick.util.api.retrieve(doc, awayLineupArgs, { cache_lifetime: 'session' },
					  Foxtrick.preventChange(doc, function(awayXml) {
						...
						Foxtrick.map(function(pair) {
							...
							Foxtrick.map(function(player) {
								...
								var lastName = player.getElementsByTagName('LastName')[0]
									.textContent;
								var nickName = player.getElementsByTagName('NickName')[0]
									.textContent;
								if (!collection[id]) {
									collection[id] = { 'name': firstName + (nickName ?
									                  (' ' + nickName) : '') + ' ' + lastName};
								}
							...
							var starting = xml.getElementsByTagName('StartingLineup')[0]
							...
							Foxtrick.map(function(player) {
								...
								var name = firstName + (nickName ? (' ' + nickName) : '')
									+ ' ' + lastName;
							...
							Foxtrick.map(function(sub) {
								...
								var subId = sub.getElementsByTagName('SubjectPlayerID')[0]
									.textContent;
								// substitute player
								var objId = sub.getElementsByTagName('ObjectPlayerID')[0]
									.textContent;
									...
									if (collection[objId] == undefined) {
										...
										Foxtrick.util.api.retrieve(doc, playerArgs,
										  ...
										  function(playerXml) {
											...
											// add in line up
											subNode.appendChild(playerTag(objId,
											                    collection[objId].name));
						...
						var report = Foxtrick
							.createFeaturedElement(doc, Foxtrick.modules.MatchReportFormat, 'div');
						...
						Foxtrick.map(function(evt) {
							...
							if (evtMarkup != '') {
									...
									if (FoxtrickPrefs.isModuleOptionEnabled('MatchReportFormat',
										...
										if (typeof(eventTypes[evtType]) == 'object') {
											...
											if (isEventTeam) {
												if (eventTypes[evtType]['team'])
													for (var i = 0;
													     i < eventTypes[evtType]['team'].length; ++i)
														createEventIcon(
															icons[eventTypes[evtType]['team'][i]],
															title, title);
											}
											else {
												if (eventTypes[evtType]['other'])
													for (var i = 0;
													     i < eventTypes[evtType]['other'].length; ++i)
														createEventIcon(
															icons[eventTypes[evtType]['other'][i]],
															title, title);
											}
										}
										//simple case, display icon for team
											...
											if (icons[eventTypes[evtType]] instanceof Array)
												for (var i = 0; i < icons[eventTypes[evtType]].length;
												 ++i)
													createEventIcon(
														icons[eventTypes[evtType]][i], title, title);
											else {
												createEventIcon(icons[eventTypes[evtType]], title,
												                title);
											}
										}
								...
								Foxtrick.util.sanitize.addHTML(doc, evtMarkup, content);
								// loaded markup from trusted source: https://chpp.hattrick.org
								...
								var indicatorList = [
									...
									},
									{
										...
										'text': 'halfTime',
										'func': function() {
											return (eventTypes[evtType] == 'possession') &&
												(evtMin == '45');
										}
									},
									{
									...
									},
									{
										...
										'text': 'extraTime',
										'func': function() {
											return eventTypes[evtType] == 'extraTime';
										}
									},
									{
										...
										'func': function() {
											return eventTypes[evtType] == 'penaltyShootOut';
										}
									},
									{
										'class': 'result',
										...
										'before': true,
										'func': function() {
											return eventTypes[evtType] == 'result';
										}
									}
								];
