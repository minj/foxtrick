...
Foxtrick.util.module.register({
	...
	PAGES : ['matchOrder'],
	OPTIONS : ["ShowSpecialties", "DisplayRatingsAlongside"],
	...
	OPTIONS_CSS : [ Foxtrick.InternalPath + "resources/css/match-order-show-specialties.css",
					Foxtrick.InternalPath + "resources/css/match-order-display-ratings-alongside.css"],
	...
	run : function(doc) { 
		...
		var showLevelNumbers = function(ev) { 
			...
			var copyRatings = function(ev) {
				...
				var select = doc.getElementById('matchSelect');
				var otherMatchId = select.value;
				if (otherMatchId > 0) {
					text += Foxtrickl10n.getString('matchOrder.comparedTo')+'\n';
					text += select.options[select.selectedIndex].textContent + ' [matchid=' + otherMatchId + ']\n';
