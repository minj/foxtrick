			...
			for (var i = 0; i < events.length; i++) {
				...
				var indicatorList = [
					...
					},
					{
						...
						'text': 'halfTime',
						'func': function() {
							return (eventTypes[evtType] == 'possession') && (evtMin == '45');
						}
					},
					{
						'class': 'full-time',
						'text': 'fullTime',
						'func': function() {
							return (eventTypes[evtType] == 'possession') && (evtMin == '90');
						}
					},
					{
						'class': 'extra-time',
							...
							if (isEventTeam) {
								if (eventTypes[evtType]['team'])
									for (var i = 0; i < eventTypes[evtType]['team'].length; ++i)
										createEventIcon(icons[eventTypes[evtType]['team'][i]],
										                title, title);
							}
							else {
								if (eventTypes[evtType]['other'])
									for (var i = 0; i < eventTypes[evtType]['other'].length; ++i)
										createEventIcon(icons[eventTypes[evtType]['other'][i]],
										                title, title);
							}
						}
						//simple case, display icon for team
							...
							if (icons[eventTypes[evtType]] instanceof Array)
								for (var i = 0; i < icons[eventTypes[evtType]].length; ++i)
									createEventIcon(icons[eventTypes[evtType]][i], title,
									                'Event Id ' + evtType + ': ' +
									                eventTypes[evtType]);
							else {
								createEventIcon(icons[eventTypes[evtType]], title, 'Event Id ' +
